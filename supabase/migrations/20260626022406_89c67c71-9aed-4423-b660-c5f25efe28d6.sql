...
BEGIN
  ...
  IF p_is_correct AND p_points_delta > 0 THEN
  ...
  ELSIF NOT p_is_correct AND p_points_delta < 0 THEN
    INSERT INTO points_log (user_id, action_type, points, description)
    VALUES (p_user_id, 'wrong_answer', GREATEST(p_points_delta, -100), 'Wrong answer penalty');
  END IF;
